...
int ASFParser::asf_parse_header() {
    ...
    if (header->subobjects > 0) {
        ...
        for (i=0; i<header->subobjects; i++) {
            ...
            if (tmp == NULL) return ASF_ERROR_OUTOFMEM;
...
int ASFParser::asf_parse_index_index() {
    ...
    for (i = 1; i < ASF_MAX_STREAMS; i++) {  // index 0 is invalid
        if (ASF_STREAM_TYPE_VIDEO == file->streams[i].type) break;
    }
    unsigned choose_index = 1;  // use the first stream index offsets on default
    if (i < ASF_MAX_STREAMS) choose_index = i;  // use video stream index offsets when has video
    ALOGD("asf_parse_index_index: choose_index %u isVideo %d",
            choose_index, (ASF_STREAM_TYPE_VIDEO == file->streams[choose_index].type));
    for (i = 0; i < index_entry_count; i++) {
        index->index_block->index_entry[i].offset = ASFByteIO::asf_byteio_getDWLE(entry_data +
                i * (file->header->index_parameters->index_specifiers_count * 4) + 4 * (choose_index - 1));
