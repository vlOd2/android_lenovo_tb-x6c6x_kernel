	...
	while (readLen == 0 && leftCnt > 0) {	/* got nothing, wait for STP's signal */
		...
		if (waitRet == 0) {
			...
			if (!mtk_wcn_stp_is_sdio_mode()) {
				printk(KERN_EMERG "#!#!#!#!#!#!# [DOCKER_DBG] tried to call mtk_wcn_consys_poll_cpucpr_dump");
				//mtk_wcn_consys_poll_cpucpr_dump(5, 1);
			}
			else
				stp_dbg_poll_cpupcr(5, 1, 1);
			if ((leftCnt == loopCnt - 1) && !mtk_wcn_stp_is_sdio_mode()) {
				printk(KERN_EMERG "#!#!#!#!#!#!# [DOCKER_DBG] tried to call mtk_wcn_consys_pc_log_dump");
				//mtk_wcn_consys_pc_log_dump();
			}

			if (!mtk_wcn_stp_is_sdio_mode()) {
				printk(KERN_EMERG "#!#!#!#!#!#!# [DOCKER_DBG] tried to call mtk_wcn_consys_stp_btif_logger_ctrl");
