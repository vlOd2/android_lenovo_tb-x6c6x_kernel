...
extern void __init __iomem * __early_ioremap(resource_size_t phys_addr, unsigned long size, pgprot_t prot);

static void __init * fb_early_memremap_prot(resource_size_t phys_addr, unsigned long size, unsigned long prot_val)
{
...
static __ref void *simplefb_earlycon_map(unsigned long start, unsigned long len)
{
	...
	fb_prot = PAGE_KERNEL;
	return fb_early_memremap_prot(fb_base + start, len, pgprot_val(fb_prot));
}
